...
void merge_chanal(char const *sours1, char const *sours2,char const *res){
    MagickWand *mw_1,*mw_2,*mw_res;
    PixelIterator *imw_1,*imw_2,*imw_res;
    PixelWand **pmw_1,**pmw_2,**pmw_res;
    Quantum qr_1,qg_1,qb_1;
    ...
    for(y = 0;y < height; ++y){
        ...
        for (x = 0;x < (long)width; ++x){
            rnd_r = rand()%2;//
            rnd_b = rand()%2;//FIX ME IT MUST BE SIMPLIER
            rnd_g = rand()%2;//
            qr_1 = rnd_r ? PixelGetRedQuantum(pmw_1[x])   : PixelGetRedQuantum(pmw_2[x]);
            qg_1 = rnd_g ? PixelGetGreenQuantum(pmw_1[x]) : PixelGetGreenQuantum(pmw_2[x]);
            qb_1 = rnd_b ? PixelGetBlueQuantum(pmw_1[x])  : PixelGetBlueQuantum(pmw_2[x]);
