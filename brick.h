...
void brick_mix_pics(const char* sours1,const char* sours2,const char* res){
    ...
    width = MagickGetImageWidth(mw_1);
    height = MagickGetImageHeight(mw_1);
    if (width != MagickGetImageWidth(mw_2) || height != MagickGetImageHeight(mw_2))
    	return;
    ...
    for(y = 0;y < height; ++y){
        ...
        for (x = 0;x < (long)width; ++x){
        	if (matrix[y][x]==1){
            	...
            	qr_1 = PixelGetRedQuantum(pmw_2[x]);
            	qg_1 = PixelGetGreenQuantum(pmw_2[x]);
            	qb_1 = PixelGetBlueQuantum(pmw_2[x]);
            }
